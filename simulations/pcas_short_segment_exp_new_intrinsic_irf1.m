function [system] = pcas_short_segment_exp_new_intrinsic_irf1 (z,maxordernle,hanklesize,delayinput,orderselectmethod,intrinsicTorque,reflexTorque)
...
%          {'stationarity_check' 0 '1 to verify and 0 to skip'}...
%      };
...
if nargin == 5
    %In future check to see if they have the same switching onset and length
    intrinsicTorque = output;
    reflexTorque = output;
else
    intrinsicTorque = intrinsicTorque.dataSet;
    intrinsicTorque = intrinsicTorque - mean(intrinsicTorque);
    reflexTorque = reflexTorque.dataSet;
    reflexTorque = reflexTorque - mean(reflexTorque);
end
...
segLength(N<1) = [];
N(N<1) = [];
totalSampleNumber = sum(segLength);
...
intrinsicTorqueCat = zeros(sum(segLength),1);
reflexTorqueCat = zeros(sum(segLength),1);
pointer = 1;
p = length(segLength);
...
for i = 1 : p
    ...
    intrinsicTorqueCat(pointer:pointer+segLength(i)-1) = intrinsicTorque(onsetPointer(i):endpointer(i));
    reflexTorqueCat(pointer:pointer+segLength(i)-1) = reflexTorque(onsetPointer(i):endpointer(i));
...
if stationarity_check
    ...
    for j = 1 : length(removeFlag)
        ...
        intrinsicTorqueCat(sw(removeFlag(j)):sw(removeFlag(j)+1)-1) = [];
        reflexTorqueCat(sw(removeFlag(j)):sw(removeFlag(j)+1)-1) = [];
...
[intrinsicTorqueCat,~,~,~] = decimate_segment(intrinsicTorqueCat,switch_time(1:end-1),decimation_ratio);
intrinsicTorqueCat = intrinsicTorqueCat - mean(intrinsicTorqueCat);

[reflexTorqueCat,~,~,~] = decimate_segment(reflexTorqueCat,switch_time(1:end-1),decimation_ratio);
reflexTorqueCat = reflexTorqueCat - mean(reflexTorqueCat);
...
%last m+1 values are B and D elements
        ...
        intrinsicTorqueCat = segdat(intrinsicTorqueCat,'domainIncr',ts,'onsetPointer',switch_time(1:end-1),'segLength',segLength);
        reflexTorqueCat = segdat(reflexTorqueCat,'domainIncr',ts,'onsetPointer',switch_time(1:end-1),'segLength',segLength);
        
        vaf_tot = vaf(tqT_noisy,tqT);
        vaf_I = vaf(intrinsicTorqueCat,tqI);
        vaf_R = vaf(reflexTorqueCat,tqR);
...
%         pos_seg = nldat(pos_seg,'domainIncr',ts);
    ...
    intrinsicTorqueCat = segdat(intrinsicTorqueCat,'domainIncr',ts,'onsetPointer',switch_time(1:end-1),'segLength',segmentLength);
    reflexTorqueCat = segdat(reflexTorqueCat,'domainIncr',ts,'onsetPointer',switch_time(1:end-1),'segLength',segmentLength);
