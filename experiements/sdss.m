function [system] = sdss (z,order,hankle_size,delay,orderdetection,intrinsicTorque,reflexTorque)
...
if nargin == 5
    ...
    intrinsicTorque = nldat(trq,'domainIncr',ts * decimation_ratio);
    reflexTorque = nldat(trq,'domainIncr',ts * decimation_ratio);
    totalTorque = nldat(trq,'domainIncr',ts * decimation_ratio);
else
    %intrinsicTorque = intrinsicTorque.dataSet;
    intrinsicTorque = intrinsicTorque - mean(intrinsicTorque);
    intrinsicTorque = decimate(intrinsicTorque,decimation_ratio);
    %reflexTorque = reflexTorque.dataSet;
    reflexTorque = reflexTorque - mean(reflexTorque);
    reflexTorque = decimate(reflexTorque,decimation_ratio);
    totalTorque = nldat(trq,'domainIncr',ts * decimation_ratio);
end
...
if isnumeric(orderdetection)
    n = orderdetection;
else
    n = orderselect(Sn,orderdetection);
end
[AT, CT] = destac(R,n);
...
if n>0
    ...
    if isnumeric(orderdetection)
        ...
        hankle_size,'orderSelectMethodLE','preset','orderLE',orderdetection);
    ...
    if isempty(reflex{2}.A)
        ...
        tqT = tqI + tqR;
        vafIntrinsic = vaf(intrinsicTorque,tqI);
        vafReflex = vaf(reflexTorque,tqR);
        vafTotal = vaf(totalTorque,tqT);
        vafs = [vafTotal.dataSet;vafIntrinsic.dataSet;vafReflex.dataSet];
    end
end
if n==0
